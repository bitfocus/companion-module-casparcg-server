...
const { Regex, InstanceBase } = require('@companion-module/base')
...
class instance extends InstanceBase {
			...
			self.socket.on('receiveline', function (line) {
				...
				// Current line is a single-line response to last message
					...
					if (self.error_code !== undefined) {
						...
						if (
							self.response_callback[self.response_current] !== undefined &&
							self.response_callback[self.response_current].length
						) {
	...
	action(action) {
		...
		if (out !== undefined) {
			self.log('debug', 'sending tcp ' + out + ' to ' + self.config.host)
